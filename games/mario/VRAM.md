...
| 0xbe-0xc7 | the pause card's second glyph run, `kHudGlyphChars2` (MAVQIT>CNP) (`kTileHudLetterSecond`, 10 tiles) | `assets_load_hud_font` | level play (pause) | the sprite gap between bowser's jaw and the toad, filled |
| 0xc8-0xcf | **FREE as bg** (8 ids) but the toad's sprite bytes: no bg data may load here | — | — | the rest of this span is empty. note that 0x96-0xbd is bowser on the **sprite** side of this bank, and a bank-1 bg id at or above 0x80 shares its bytes with the sprite id of the same number — which is why m23's run below starts at 0xd0 rather than at 0x8d |
