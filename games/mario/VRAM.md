...
| 0x78-0x7b | the pennant coming down the pole: the flag head's four bg tiles again, as two 8x16 sprites (`kTilePennant`, the same `gen/flag_head.c`) | `assets_load_scenery_tiles` | level play (clear sequence) | the family is stored column by column, which is 8x16 sprite order, so the one bg array loads both pairs. worn in `kPalCoin`, which `flow_flag_arm` retints to the sky slot's white/blue/black for the slide - no coin can pop during the clear, and the item loader puts the gold back at the next level |
| 0x7c-0x7f | **FREE** (4 ids) | — | — | the tail of what big mario's old 0x60-0x7f block gave back |
...
| 0x7e-0x7f | **FREE** (2 ids) | — | — | between the side-pipe run and the hud font |
| 0x80-0x95 | hud row glyphs: 10 digits, blank, coin icon, then the letters `kHudGlyphChars` (x and the pause overlay's WORLD-ESU) (`kTileHudDigitFirst`-`kTileHudLetterFirst`, 22 tiles) | `assets_load_hud_font` | level play only (the window-layer hud row is drawn only during play) | shares bytes with bank-1 **sprite** ids 0x80-0x8c — see the sprite table |
| 0x96-0xbd | **FREE as bg** (40 ids) but bowser's bank-1 sprite bytes: no bg data may load here | — | — | the hud run stops at 0x95 for this reason |
| 0xbe-0xc4 | the pause overlay's second glyph run, `kHudGlyphChars2` (MAVQIT>) (`kTileHudLetterSecond`, 7 tiles) | `assets_load_hud_font` | level play (pause) | the sprite gap between bowser's jaw and the toad |
| 0xc5-0xcf | **FREE** (11 ids) | — | — | 0xc5-0xc7 share no sprite bytes; 0xc8-0xcf are the toad's sprite bytes, so bg data may not load there; the rest of this span is empty. note that 0x96-0xbd is bowser on the **sprite** side of this bank, and a bank-1 bg id at or above 0x80 shares its bytes with the sprite id of the same number — which is why m23's run below starts at 0xd0 rather than at 0x8d |
...
m26's 1-3 pass added **no bg id at all** and gave two back: `0x10-0x11`. it re-cut the tree canopy
and the trunk off `games/mario/art/ref/smbd_ch_1-3.png`
(`games/mario/tools/rip_tiles.py --level 1-3`, `gen/tree.c` and `gen/trunk.c`) and the capture said
the canopy's right cap is its left twin mirrored to the pixel — the first right cap in this game
that is — so `kBlockTreeTopR` lost its two dedicated tiles and took `kCamAttrXFlip` instead, and
`kTileTrunk` moved down from 0x11 onto 0x0f to keep the run contiguous. it added no kind and no
palette either: every colour the 1-3 capture paints is already in the eight overworld slots, the
canopy's four being exactly `kCamPalPipe`'s and the trunk's two `kCamPalBrick`'s. the only thing it
...
| 0x52-0x63 | the score popup's strip: nine 8x16 columns, glyph on top and blank below - the left halves 10 20 40 50 80, the right halves 0 and 00, and the 1-UP's pair (`kTilePopupFirst`, `gen/popup.c`, drawn by `tools/draw_popup.py`) | `popup_art_load` (bank 6) | level play | a label is one left column beside one right one, so smb's eleven floatey values are pairs out of nine columns. the 1-UP pair's ids are numerically the bank-0 koopa's, which a host test reading tile numbers should keep in mind; the digits below 0x60 are unique across both banks |
| 0x64-0x73 | **FREE** (16 ids) | — | — | bg ids below 0x80 do not share bytes with a sprite id (see addressing rule), so bank 1's bg scenery run is no obstacle here |
...
| 4-8 | brick debris (4) then the fireball's puff (1) | `debris.c` | the world map borrows 4-11 for its four node markers and the toad room 4-7 for the retainer, both on screens with no debris, no items and no enemies; the clear borrows 4-5 for the pennant sliding down the pole (`flow_flag_arm` calls `debris_clear` first - nothing can break a brick once the pole has him) |
...
| past the pool, under 24 | **the score popup** — the two slots right after what the enemy pool used this frame (`enemies_oam_top()` before it is published), taken only when both lie under the hazards floor | `popup.c`, called from the end of `enemies_draw` | one label at a time, 48 frames. it re-places itself every frame as the pool grows and shrinks under it, parks a slot only if the pool has not already climbed into it, and is simply not drawn on a frame with no room - the same degradation the hazards pool takes |
