...
|---|---|---|---|
| walk / run cycle | three poses, each held for a timer picked by speed: `PlayerAnimTmrData` `02 04 07` (6192), chosen at `GetPlayerAnimSpeed` (6195-6224): abs xspeed >= 0x1c -> 2 frames a pose, >= 0x0e -> 4, else 7; the cycle itself at `AnimationControl` (14660-14676), extent 3 | `player.c` `step_anim`: `anim_accum += speed_abs`, pose steps every `kWalkAnimStepSubpx` (48) subpixels - at the run cap (40 subpx) that is a pose every 1.2 frames, at the walk cap (24) every 2, which is why his arms blur at a run | approximate -> fixed in this pr: `PlayerAnimTmrData` verbatim. the poses themselves were wrong too: rip_sprites.py had read the sheet's rows as gameplay order and put the jump and the death pose into the small walk cycle (and a walk frame into the jump, the skid into a walk frame); `PlayerGraphicsTable` (14399) tile sharing identified every pose and the rectangles moved |
