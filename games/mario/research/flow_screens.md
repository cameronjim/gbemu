...
| after the clear | smb goes to the next area's world/lives card once `DelayToAreaEnd` has waited out the fanfare; smbd's original mode does the same. no course clear screen in either | a blue COURSE CLEAR / TIME / SCORE card held 90 frames | gone: the fanfare ends and the map comes up, the file recorded on the way |
