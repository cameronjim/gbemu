...
|---|---|---|---|
| world / lives card | `DisplayIntermediate` before every level and every respawn, never after a pipe: WORLD x-y, the player sprite, x lives, held `ScreenTimer` 7 intervals (147 frames), silent | missing | present, `card_lives` in title.c, `kStateLivesCard`; text stands in for the sprite; `music_quiet` lets a death jingle finish first |
...
- smbd draws the player sprite on the lives card and condenses its game over screen "like the pause
  screen" (mariowiki). neither layout was measured; both are text on the card sky here.

## tests

`mario_lives_card_precedes_the_level`, `mario_clear_pays_the_time_an_interval_a_frame_with_smb_s_tick`,
