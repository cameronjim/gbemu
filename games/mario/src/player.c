...
void player_draw(uint16_t cam_x, uint8_t cam_y, uint8_t palette) {
    ...
    if (crouched != 0U) {
        ...
        move_sprite(kSpriteMarioL, 0, 0);
        move_sprite(kSpriteMarioR, 0, 0);
        draw_row(kSpriteMarioLowL, (uint8_t)(kTileSuperLowerFirst + kFrameCrouch * kSuperTilesPerFrame),
                 prop, sx, (int16_t)(sy + kCrouchInsetPx));
        return;
    }
    // every standing pose shares the upper slab, so only the legs cost tiles of their own
