...
// 1-3's trees: a canopy of cap/middle/cap, solid on every side the way smb's tree tops are, over a
// trunk of scenery hanging from the canopy's middle columns down to the bottom row
...
constexpr uint8_t kBlockFloorTable[kBlockKindCount] = {
    ...
    0,
    // 1-3's trees: the canopy is solid on every side, the trunk under it is scenery
...
// 1-3's red paratroopa, the roster kind the wings are still owed
    ...
    void enemy_spawn() {
        ...
        while (cursor < roster_count) {
            ...
            *slot = EnemySlot{};
            ...
            // enemies.c's own line: the flyer is m9's, so 1-3's red paratroopas come in as the red
            // koopa they stomp down into, on the cell the roster names
            if (slot->kind == kEnemyKoopaParaRed) {
                slot->kind = kEnemyKoopaRed;
            }
            slot->pos_x = px;
