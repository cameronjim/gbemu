...
TEST_CASE("mario_star_invincibility") {
    ...
    // count is waited on rather than read a fixed four frames later: the route in is a few thousand
    // frames of replay and the rom carries a hair more per-frame work than it did before the window
    // hud, so which frame the goomba walks into him can slide by a few either way
    bool took_the_goomba = false;
    for (int i = 0; i < 30 && !took_the_goomba; ++i) {
        gameboy.run_frame();
        ++elapsed;
        REQUIRE(!at_start_cell(gameboy));
        took_the_goomba = goombas_on_screen(gameboy) < goombas_before;
    }
    REQUIRE(took_the_goomba);
