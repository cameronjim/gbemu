...
// SELECT FILE's own heading row, kFileHeadRow in mario.h; the rest of the kFile* block is down with
// the front-end tests, but the title's start probe needs this one up here
...
// the card carries no prompt any more, so this is what says start still reaches the file select
TEST_CASE("mario_title_start_opens_file_select") {
    ...
    // "SELECT FILE" is eleven glyphs centered across the 20 columns, so it spans 4 through 14
...
// the rest of the SELECT FILE card's rows and its cursor column, from the kFile* block in mario.h.
// every slot line is padded to kFileLineWidth glyphs, so the cursor always sits in the same column
...
// the file and erase cards share card_begin's sky, the title has the generated art's own and the
// map a third, so the three of them together are exactly the front end
bool sky_is_gameplay(int sky) {
    return sky != kSkyTitle && sky != kSkyCard && sky != kSkyMap;
}
...
TEST_CASE("mario_deliberate_presses_still_reach_a_level") {
    ...
    step_screen(gameboy, gb::Button::Start);
    run(gameboy, kScreenSettleFrames);
    REQUIRE(sky_color(gameboy) == kSkyCard); // the file card paints card_begin's sky, not the art
