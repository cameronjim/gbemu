...
def compile_grid(bible, level_type):
    ...
    for t in terrain:
        if t["kind"] == "ground":
            ...
            # when a level's own per-level block list (level_N_objects.c, see the bank note on
            # LEVEL_MAX_BLOCKS in blocks.c) has no more room: every level's lists are copied into one
            # ram arena sized to the largest, so a longer list costs every level wram, and blocks.c
            # keeps per-block state arrays sized to LEVEL_MAX_BLOCKS on top of that
...
def write_objects(out_dir, slug, level, source_path):
    # the lists live beside the grid in the level's own bank; level.c copies the ones being played
    # into ram at level select, so the per-frame scans read ram and bank 0 holds none of this
    upper = slug.upper()
