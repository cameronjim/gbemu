...
def write_files(out_dir, slug, png_name, bank, blocks, defines, externs, cgb):
...
def emit_tiles(args, img):
    ...
    write_files(args["out_dir"], slug, os.path.basename(args["png"]), args["bank"], [block],
...
def emit_screen(args, img):
    ...
    write_files(args["out_dir"], slug, os.path.basename(args["png"]), args["bank"], blocks,
                defines, externs, cgb=True)
    if args["report"]:
